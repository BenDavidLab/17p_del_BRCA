...
# Read the CSV file
TCGA <- read.csv(here::here("data","Survival_TCGA.csv"))
annotations = list("OS" = c("Overall survival"),"DSS" = c("Disease specific survival"), 
                   "DFS" = c("Disease free survival"),  "PFS" = c("Progression free survival"))

for (annotation in names(annotations)) {
  annotation_df <- TCGA[, c("PATIENT_ID", "Genotype", "Molecular_subtype", paste0(annotation, "_MONTHS"), paste0(annotation, "_STATUS"))]
  # Rename columns
  ...
  subtypes <- subtypes[subtypes!='NC'& subtypes!='']
  ...
  for (subtype in subtypes){
    ...
    # Print the plot
    # print(g)
    if (!file.exists(here::here("TCGA","survival"))) {
      # Create the directory
      dir.create(here::here("TCGA","survival"), recursive = T)
    }
    if (!file.exists(here::here("TCGA","survival", annotation))) {
      # Create the directory
      dir.create(here::here("TCGA","survival", annotation))
    }
    if (!file.exists(here::here("TCGA","survival", annotation, "Del_vs_non_del"))) {
      # Create the directory
      dir.create(here::here("TCGA","survival", annotation, "Del_vs_non_del"))
    }
    pdf(here::here("TCGA","survival", annotation, "Del_vs_non_del", paste0("Del_vs_Non_del_17p_", subtype, "_120_month.pdf")))
  ...
  for (subtype in subtypes){
    ...
    # Print the plot
    # print(g)
    if (!file.exists(here::here("TCGA","survival", annotation, "Genotypes"))) {
      # Create the directory
      dir.create(here::here("TCGA","survival", annotation, "Genotypes"))
    }
    pdf(here::here("TCGA","survival", annotation, "Genotypes", paste0("4_Genotypes_", subtype, "_120_month.pdf")))
    print(g, newpage = FALSE)
    dev.off()
  }
  ...
  for (subtype in subtypes){
    ...
    # Print the plot
    # print(g)
    pdf(here::here("TCGA","survival", annotation, "Genotypes", paste0("3_Genotypes_", subtype, "_120_month.pdf")))
    print(g, newpage = FALSE)
    dev.off()
  }
