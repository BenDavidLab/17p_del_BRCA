Code for the "Chromosome-arm 17p loss renders breast cancer cells vulnerable to AURKB inhibition".

**Survival analysis**

To recreate the TCGA/Metabric survival plots run Survival_analysis_TCGA_120_month_limited.R/Survival_analysis_METABRIC_120_month_limited.R. The results are saved to TCGA/survival or Metabric/survival directory.
...
DESeq2 is used for RNA-Seq (TCGA) count data and limma for microarray data (METABRIC). The geneset collections to use in the GSEA are hard-coded and can be changed in the code.

[Additional files:]{.underline}
